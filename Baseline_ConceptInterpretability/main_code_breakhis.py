...
class Classifier(nn.Module):
    ...
    def __init__(self, name: str = "model"):
        ...
        self.transform = transforms.Compose([transforms.Resize(128,antialias = True),transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    ...
    def fit(
        ...
        lr: int = 1e-03,   #1e-04 => Atingue os melhores resultados 1e-5 só apresenta uma ligeira melhoria
        n_epoch: int = 100, #200
        patience: int = 20,
...
def train_model(run, train_loader, val_loader, test_loader):
  if not os.path.exists("models/breakhis"):
    os.makedirs("models/breakhis")
  model = Classifier("model"+ str(run))
  device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
  model.fit(device,train_loader, val_loader, "models/breakhis")

  #Get the best model from the previous run
  model = Classifier("model"+ str(run))
  model.load_state_dict(torch.load("models/breakhis/model"+ str(run) +".pt"), strict=False)
  model.to(device)
  model.eval()
...
def concept_dataset(run,concept_name, np_images,label, train_slip, test_slip): 
    ...
    for k in range(0,len(labels_save)):
        if(labels_save[k].split("_")[2].split("-")[0] == concept_name):
            pos.append(k)
            labels_y.append(1)
        else:
            neg.append(k)
...
def concept_accuracy(
    images,label, run,
    plot: bool,
    save_dir: Path = "results/breakhist/concept_accuracy",
    data_dir: Path = "data/breakhist",
    model_dir: Path = "models/",
    model_name: str = "model_ben_mal",
) -> None:
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

    concept_names = ["A", "F", "TA","PT","DC", "LC", "MC","PC"]
    ...
    for concept_name in concept_names:  
        ...
        X_train, X_test, y_train, y_test = concept_dataset(run,concept_name, images,label, 400, 50)
...
def global_explanations(
    ...
    for concept_id in range(0,len(concept_names)):
        ...
        images_train, images_test, labels_train, labels_test = concept_dataset(run,concept_names[concept_id], images,label, 400, 50)
    ...
    a, images_test, b, c, classes_test, d = pre_datasets(run,images,label)
...
images = np.load('images_BreaKHis.npy')
label = np.load('labels_BreaKHis.npy')
...
for run in range(0,30):  # Go until 30
  ...
  concept_accuracy(images,label,run,True,"results/breakhist" + str(run) + "/concept_accuracy","data/breakhist","models/ben_mal/","model" + str(run))
  global_explanations(images,label,run,1,True,"results/breakhist" + str(run) + "/global_explanation","data/breakhist","models/ben_mal/","model" + str(run))
