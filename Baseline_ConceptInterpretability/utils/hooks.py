...
def create_forward_hook(name: str, save_dir: Path):
    ...
    """
    if os.path.exists(save_dir + "/" + str(name) + ".npy"):  # Ensure that the hook is not going to append an existing file
        os.remove(save_dir + "/" + str(name) + ".npy")
    def hook(module, input, output):
        ...
        #with (save_dir + "/" + str(name) + ".npy").open('rb') as f:
        np.save(save_dir + "/" + str(name) + ".npy", save_arr)
    return hook
