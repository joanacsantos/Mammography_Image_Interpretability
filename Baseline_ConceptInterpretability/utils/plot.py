...
def plot_attribution_correlation(results_dir: Path, dataset_name: str, filtered_concepts: list = None,
    ...
    sns.color_palette("colorblind")
    sns.set_style("white")
    attribution_dic = np.load(results_dir + "/" +"attributions.npz", allow_pickle=True)
...
def plot_grayscale_saliency(images: torch.Tensor, saliency: np.ndarray, plot_indices: list,
    ...
    for ax_id, example_id in enumerate(plot_indices):
        ...
        ax.axis('off')
        sns.heatmap(np.sum(np.squeeze(sub_saliency), axis=0), linewidth=0, xticklabels=False, yticklabels=False,
...
def plot_time_series_saliency(tseries: torch.Tensor, saliency: np.ndarray, plot_indices: list,
                              results_dir: Path, dataset_name: str, concept_name: str) -> None:
    sns.set(font_scale=1)
    sns.color_palette("colorblind")
    sns.set_style("white")
