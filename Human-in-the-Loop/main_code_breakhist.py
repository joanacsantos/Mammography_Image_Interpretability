...
def pre_datasets(np_images,label): 
    ...
    for k in range(0,len(labels_save)):
        if(labels_save[k].split("_")[1] == 'B'):
            classes_class.append(0)
        elif(labels_save[k].split("_")[1] == 'M'):
            classes_class.append(1)
           
    classes1 = []
    for k in range(0,len(labels_save)):
        if(labels_save[k].split("_")[2].split("-")[0] == "A"):
            classes1.append(0)
        elif(labels_save[k].split("_")[2].split("-")[0] == "F"):
            classes1.append(1)
        elif(labels_save[k].split("_")[2].split("-")[0] == "TA"):
            classes1.append(2)
        elif(labels_save[k].split("_")[2].split("-")[0] == "PT"):
            classes1.append(3)
        elif(labels_save[k].split("_")[2].split("-")[0] == "DC"):
            classes1.append(4)
        elif(labels_save[k].split("_")[2].split("-")[0] == "LC"):
            classes1.append(5)
        elif(labels_save[k].split("_")[2].split("-")[0] == "MC"):
            classes1.append(6)
        elif(labels_save[k].split("_")[2].split("-")[0] == "PC"):
            classes1.append(7)
            
    print('Images for Training: ' + str(len(classes_class)) + '-> ' + str(len(np.where(np.array(classes_class) == 0)[0])) +' benign and ' + str(len(np.where(np.array(classes_class) == 1)[0])) + ' malignant images')
...
def train_model(run, train_loader, test_loader):
  if not os.path.exists("models/2class_new"):
    os.makedirs("models/2class_new")
  model = Classifier("model"+ str(run))
  device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
  model.fit(device,train_loader, "models/2class_new")

  #Get the best model from the previous run
  model = Classifier("model"+ str(run))
  model.load_state_dict(torch.load("models/breakhist/model"+ str(run) +".pt"), strict=False)
  model.to(device)
  model.eval()
...
run = 0
images, classes_class, classes = pre_datasets(images,label)

print(len(classes_class))
print(len(classes))
for train_index, test_index in skf.split(images, classes):
  ...
  write_line_to_csv(
      "results/","Test_breakhist_10k.csv",
            {
                "RUN": (run + 1),
